...
%
  \begin{itemize}
    ...
    \item a \PRF/ distinguisher algorithm $A'$ making
       $\bigl(1 + \lfloor\ell/64\rfloor\bigr)
...
Let $\collisionbound = 8\lceil\ell/16\rceil/2^{212}$ be a bound on
 the collision probability of $\Poly1305/^2$.
...
\begin{align*}
  ...
  &= \frac{(E + D)^2 + D + \binom{E}{2}}{2^{192}}
     + 8\lceil\ell/16\rceil \frac{D E + \binom{E}{2}}{2^{212}}.
\end{align*}

...
The algorithm $A'$ performs
 $\sum_n (1 + \lfloor\ell/64\rfloor) \bigl(E(n) + D(n)\bigr)$ oracle
