...
\begin{description}
  \item[\normalfont $c = \DaenceEncrypt(k, a, m)$] takes a key $k$
     ...
     a string $a$ of fewer than $2^{38}$ bytes of associated data, and
     a message $m$ of fewer than $2^{38}$ bytes ($256\,\mathrm{GB}$).
  ...
  \item[\normalfont $m' = \DaenceDecrypt(k, a', c')$] takes a key $k$,
     a string $a'$ of fewer than $2^{30}$ bytes of associated data,
...
\begin{enumerate}
  ...
  \item You must process no more than a total of $2^{52}$ messages,
     including legitimate messages and forgery attempts, with any
     single key.
...
To \term{encrypt}, given
 a 96-byte key $k$,
 associated data $a$ of fewer than $2^{30}$ bytes, and
 a message $m$ of fewer than $2^{30}$ bytes,
 compute:
%
\begin{enumerate}
