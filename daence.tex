...
\begin{proof}
  Let $h_x := \Poly1305/^2_{k_1,k_2}(x)$, and note that
%
  \begin{align*}
    \Pr&[(h_a, h_m) = (h_{a'}, h_{m'}) \given (a, m) \ne (a', m')] \\
    &= \Pr[h_a = h_{a'} \mathrel{\text{and}} h_m = h_{m'}
           \given a \ne a' \mathrel{\text{or}} m \ne m'] \\
    &= \Pr[h_a = h_{a'} \given a \ne a']
       + \Pr[h_m = h_{m'} \given m \ne m'].
  \end{align*}
%
  By Lemma~\ref{double-hash},
   $\Pr[h_x = h_{x'} \given x \ne x'] \leq \collisionbound(\ell_x)$.
