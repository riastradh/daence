...
The nonce-based authenticated cipher \AESGCM/~\cite{nist-sp800-38d} is
 popular, though it is fast and resistant to timing side channels only
...
We propose a deterministic authenticated cipher \DAENCE/, built out of
 a deterministic authenticated cipher using \Salsa20/ or \ChaCha/
 and the \Poly1305/ universal hash family.
...
\begin{enumerate}
  \item If you repeat an associated data string $a$, then an adversary
     has no hope of distinguishing the ciphertexts of \emph{distinct}
     messages from uniform random byte strings of the same length, but
     will notice when messages are repeated.
  ...
  \item The adversary's probability of succeeding at forgery is less
     than $2^{-32}$.
\end{enumerate}
