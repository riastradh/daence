...
void crypto_dae_salsa20poly1305(u8 *c,const u8 *m,u64 mlen,
    const u8 *a,u64 alen,const u8 *k)
{
  u8 k1[32], k2[32], ham[64], h[32], t[32], i;

  FOR(i,16) { k1[i] = k[32 + i]; k1[16 + i] = 0; }
  FOR(i,16) { k2[i] = k[48 + i]; k2[16 + i] = 0; }
  crypto_onetimeauth_poly1305(ham +  0, a, alen, k1);
...
int crypto_dae_salsa20poly1305_open(u8 *m,const u8 *c,u64 mlen,
    const u8 *a,u64 alen,const u8 *k)
{
  u8 k1[32], k2[32], ham[64], h[32], t[32], t_[32], i;
